...
로봇언어에서는 call문으로 서브 job 프로그램을 호출(call)할 수 있습니다. 서브 프로그램은 end나 return문을 수행할 때 자신을 호출한 call문의 다음 명령문 위치로 리턴(return)합니다. 예를 들어 아래 그림에서, 5번 job은 8번 job을 호출하여 수행하다가 return문을 만나서 다시 5번 job의 call문의 다음 명령문부터 수행을 계속해나가는 것을 볼 수 있습니다. 
