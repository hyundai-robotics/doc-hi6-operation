...
3. 모든 베이스 축들이 원점에 위치한 상태에서 로봇의 위치와 자세를 다양하게 움직여 20 점 이상의 위치를 측정하고 위치를 프로그램으로 기록하십시오. 이 때, 초기 3 점은 방향이 고정된 상태에서 하나의 직선 상에 존재하지 않도록 위치를 기록합니다. 일반적으로 측정 데이터의 개수가 많을수록 보다 정확한 캘리브레이션이 가능합니다.
...
4. 로봇은 고정된 상태에서 베이스 축만 움직여 위치를 측정하고 위치를 프로그램으로 기록하십시오. 
    ...
    * 각 베이스 축에 대한 위치 측정 및 기록은 연속되게 배치하십시오. 프로그램에 여러 베이스 축에 대한 위치가 섞여서 기록되면 캘리브레이션이 정상적으로 수행될 수 없습니다.
