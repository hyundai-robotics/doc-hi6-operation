...
![LCD 로봇 시스템의 기본 구성](../../_assets/image.png)
...
![제어기 앞면\(좌\) / 뒷면\(우\)](../../_assets/image_33.png)
...
![TP630 앞면\(좌\) / 뒷면\(우\)_](../../_assets/tp630/TP-hw.png)
...
| ![](../../../_assets/c7.png) | 안내 표시줄 : 조작 중 각종 안내 메시지를 보여줍니다.  ([1.2.4.7 안내 표시줄](7-guide-area.md)) |
...
| ![](../../../_assets/c5.png) | 수동모드의 초기화면에 [명령입력] 버튼이 표시됩니다. 작업 편집창에 명령문을 입력하는 용도로 사용됩니다. ([3.2.2.1 일반 명령문 입력](../../../3-programming/2-prog-edit/2-statement-input/1-gen-statement-input.md))|
...
![cmd/step 설정 시 재생 전후진 경로](../../_assets/path-cmd-step-pback-fwd-bwd.png)
...
![end 설정 시 재생 전후진 경로](../../_assets/path-end-pback-fwd-bwd.png)
...
![스텝 후진 후 전진 시 로봇 경로 변경 예](../../_assets/path-step-bwd-then-fwd.png)
...
${cont_model} 티치 펜던트 화면 우 하단에서 `조건설정` 버튼을 터치한 후 설정창에서 \[스텝 전후진시 최고속\] 과 \[자동운전 속도비율\] 옵션의 값을 설정하십시오.
...
![P-PTP 보간의 툴 끝 경로 예](../../../_assets/image_73.png)
...
![L-직선 보간 예](../../../_assets/image_48.png)
...
![C-원호보간 예1](../../../_assets/image_96.png)
...
![C-원호 보간 예2](../../../_assets/image_101.png)
...
![Accuracy에 따른 P2의 경로 변화](../../../_assets/image_53.png)
...
![Accuracy level 값에 따른 코너링 경로 생성](../../../_assets/image_79.png)
...
![Convex polygon 영역 내의 코너링 경로의 모든 점](../../../_assets/image_87.png)
...
![사용자키 영역 전환](../../_assets/tp630/user-bar/user-bar.png)
...
HOME 화면 우측의 `[R4: 사용자키]`를 눌러 `키 신호 출력` 버튼을 열 수 있습니다. 설정을 하지 않았다면, 모든 버튼이 비어있을 것입니다.
...
![사용자키 영역 전환](../../../_assets/tp630/user-bar/ubar-spotweld-cfg.png)
...
![티치 펜던트의 조그키](../../_assets/tp630/keypad-jog.png)
...
* 오른손을 이용하면 로봇 좌표계에서 로봇의 동작을 쉽게 이해할 수 있습니다.
	...
	![좌표계 방향\(좌\) / 회전 방향\(우\)](../../_assets/crd-direction.png) 
...
{% hint style="info" %}
* 축 원점 최적화 기능은 V70.02-00 버전부터 일반 사용자에게 지원되지 않습니다. 이후 버전부터 축 원점을 변경하려면 고객지원팀에 문의하여 전문가에게 의뢰하거나 엔지니어에게 문의하십시오.
{% endhint %}
...
![명령문 편집 - 인수값 입력](../../_assets/tp630/pane-prog-move-option.png)
...
![명령문의 구성 영역](../../_assets/tp630/pane-prog-section.png)
...
![영역 간 커서 이동\(좌: 주소 영역, 우: 명령문 영역\)](../../_assets/tp630/pane-prog-sectionchng.png)
...
* \[![](../../../_assets/tp630/bt-dock-softkb.png)\] 버튼을 터치하면 키보드의 위치를 화면 상단으로 이동할 수 있습니다.
...
![가동 정보](../../_assets/tp630/pane-operating.png)
...
![히스토리](../../_assets/tp630/pane-history.png)
...
![시스템 입력 - ON/OFF,값,시퀀스 상태](../../_assets/tp630/pane-system-input.png)
...
![시스템 출력 - ON/OFF/값/시퀀스 상태](../../_assets/tp630/pane-system-output.png)
...
![범용 입력 신호 - ON/OFF 상태\(좌\) / 값 상태\(우\)](../../_assets/tp630/pane-univinsig-mode.png)
...
![범용 입력 신호 - ON/OFF 상태\(좌\) / 값 상태\(우\)](../../_assets/io/io_fn_mon.png)
...
![프로그램 예약 실행 화면](../../_assets/tp630/pane-prog-reserv.png)
...
![태스크](../../_assets/tp630/pane-task.png)
...
![센서 동기 모니터링](../../_assets/tp630/pane-sensorsynch.png)
...
* \[3: "wait" 명령문 강제 해제\]: "wait" 명령문 대기중에서 `[SHIFT]`+ `WAIT 해제` 키를 이용해 대기 상태를 강제로 해제할 지 여부를 설정합니다.
...
* \[7: 입출력 신호 바이트 표시]: 입출력 신호를 바이트 단위로 표시합니다.
  * 유효로 선택한 경우, 입력 신호 할당 페이지의 설정화면이 아래와 같이 변경됩니다. <br>
  ![](../_assets/tp630/system-user-environ-byte-index.png)

{% hint style="info" %}
* PLC 중에는 신호 체계가 바이트 단위로 구성되는게 많이 있으며 이런 장비들과 인터페이스할 때 더욱 직관적으로 사용할 수 있습니다. <br>
8번째 신호인 경우는 바이트 1의 비트 0으로서 간단히 1.0으로 표현합니다. 따라서 출력 신호는 QX1.0으로 입력 신호는 IX1.0으로 표현됩니다. 
* 로봇 언어에서도 8번째 신호를 on으로 출력하기 위해서는 `do8=1`을 실행할 수도 있지만 `dob1.x0=1`의 비트 출력이 가능합니다. 즉 `fb2.do11=1`은 `fb2.dob1.x3=1`과 동일한 출력입니다.<br>
{% endhint %}
...
* \[10: 전원 투입 실행 프로그램\]: 제어기가 부팅이 완료되었을 때, 설정된 프로그램의 end 명령문까지 자동으로 실행합니다. 제어기 부팅시 전역 변수의 값이 초기화되기 원한다면 이를 별도의 프로그램으로 작성하고 해당 프로그램 번호를 등록해 두면 부팅시마다 초기화 된 값을 사용할 수 있습니다. (V70.04-00 이상에서 지원)  
...
- 협조 제어나 컨베이어 동기 기능을 사용하는 경우는 절전 모드가 동작하지 않습니다.  
...
\]
  ...
  * Dynamic Motion 비율 설정 (`0 ~ 1.0`)
    - `0` : 사용하지 않습니다. (정적 이동)
    - `0 ~ 1.0` : 동적 모션 적용 강도 조정할 수 있습니다.
...
Hi6 제어기에서 냉각팬 제어를 위해 사용합니다. 일반적으로 냉각팬 제어는 so49에 결선되어 있습니다. 출력 오류로 냉각팬이 off되면 반내온도 과열 에러가 발생할 수 있습니다. 

* 모터ON 준비완료(원격자동)

원격 자동모드에서 모터온이 가능한 상태를 출력하고자 할 때 사용합니다.
...
![작업 프로그램 실행 예](../../../_assets/image_58.png)
...
![작업 프로그램 실행 예](../../../_assets/image_50.png)
...
![사용자 좌표계 정의를 위한 3개의 기준 스텝 티칭 방법](../../../_assets/image_197.png)
...
![실링 작업 예](../../../_assets/tp630/stationary_crd_sealing.png)
...
![자동설정을 이용한 티칭 방법](../../../_assets/tp630/stationary_crd_autoset.png)
...
![로봇 형태별 플랜지 좌표계](../../../_assets/image_213.png)

툴의 각도는 플랜지 좌표계에서 방향별 자세 변환량입니다. \(X축 방향 각도: Rx / Y축 방향 각도: Ry / Z축 방향 각도: Rz\)

![툴 각도: Rotating Rx \(좌\) / Rotating Ry \(중간\) / Rotating Rz \(우\)](../../../_assets/image_211.png)
    ...
    <tr>
      ...
      <td style="text-align:left">
        <ul>
          ...
          <li>[+]/[-]: 새로운 툴 데이터를
            추가하거나 툴 데이터를
            삭제합니다. 현재 선택된 툴 번호에 해당하는 툴 데이터는 삭제가 불가합니다.</li>
          <li>툴 데이터 목록입니다.
            툴 데이터 이름을 선택하면
            상세 정보를 확인 및 편집할
            수 있습니다.</li>
...
![고부하 모드 설정화면](../../../_assets/image_high_load_mode_setting.png)
...
로봇에 현재 적용 된 툴의 부하에 따른 고부하 모드 적용 상태는 아래 그림에서와 같이 확인 할 수 있습니다.<br>

![툴 부하에 따른 고부하 모드 적용 상태 확인](../../../_assets/home_tool_no.png)
...
![로봇 형태별 각 축의 좌표계 원점](../../../_assets/image_484.png)
...
![인칭 거리 도달 후 키에서 손을 뗀 경우](../../../_assets/image_209.png)

인칭 거리에 도달하기 전에 조그키에서 손을 뗀 경우에는 조그키에서 손을 뗀 시점부터 로봇이 감속하여 정지합니다. 이때는 일반 조그 모드와 동일합니다.

![인칭 거리 도달 전 키에서 손을 뗀 경우](../../../_assets/image_216.png)
...
{% hint style="info" %}
* 본 기능은 V60.30.07 이상의 버전에서 지원합니다.    
{% endhint %}
...
3.	저장된 데이터를 확인한 후 \[초기화\] 버튼을 터치하십시오. 모든 설정 파일, 프로그램을 삭제할 수 있으며 모든 데이터도 초기 설정값으로 복원됩니다.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td style="text-align:left">모든 설정 파일의 삭제 여부입니다.<br>
      </td>
    </tr>
    <tr>
      <td style="text-align:left">
      ...
      <td style="text-align:left">모든 프로그램과 래더 파일의 삭제 여부입니다.</td>
    </tr>
    <tr>
      <td style="text-align:left">
        I/O, 릴레이
      </td>
      <td style="text-align:left">모든 입출력 신호 및 릴레이 값의 초기화 여부입니다.</td>
    </tr>
    <tr>
      <td style="text-align:left">
        변수
      </td>
      <td style="text-align:left">전역 변수로 생성된 파일의 초기화 여부입니다.
      </td>
    </tr>
    <tr>
      <td style="text-align:left">
    ...
    <tr>
      ...
      <td style="text-align:left">레이더 설정 정보의 초기화 여부입니다.
      </td>
    </tr>
  </tbody>
</table>
...
* 동기 그룹 쌍이 되는 축들은 같은 보드에 있는지 확인하십시오.
{% endhint %}
...
![축 원점 및 툴 길이 최적화 기능 교시 방법](../../_assets/image_228.png)
...
![1축 표지셔너\(좌\) / 2축 포지셔너\(우\)](../../_assets/image_244.png)
...
![부하추정 기능](../../_assets/tp630/system-calib-load.png)
...
![베이스 축 캘리브레이션](../../../_assets/image_513.png)
...
{% hint style="info" %}
* 당사는 두 가지 방식의 베이스축 캘리브레이션을 지원합니다. 첫 번째 방법은 사용자가 직접 기준점을 기록하는 [교시 기반 베이스축 캘리브레이션]이며, 두 번째 방법은 레이저 트래커를 이용하여 기준점을 측정하는 [센서 기반 베이스축 캘리브레이션]입니다. 환경에 맞게 적절한 방법을 선택하십시오.
* [센서 기반 베이스축 캘리브레이션] 기능은 V70.04-00 및 이후 버전부터 지원됩니다.
{% endhint %}
...
{% hint style="warning" %}
* 베이스 축 구성이 임의인 경우에만 베이스축 캘리브레이션을 사용할 수 있습니다.
* \[교시 기반 베이스축 캘리브레이션\]은 첫 번째 베이스 축에 한하여 캘리브레이션 기능을 사용할 수 있습니다. 첫 번째 베이스 축에 대해 축 구성을 임의로 설정하고, 나머지 베이스 축에는 축 구성을 임의로 설정하지 마십시오.
* \[센서 기반 베이스축 캘리브레이션\]은 모든 베이스 축에 대하여 캘리브레이션 기능을 적용합니다. 모든 베이스 축에 대해 축 구성을 임의로 설정하십시오.
{% endhint %}
...
3. 모든 베이스 축들이 원점에 위치한 상태에서 로봇의 위치와 자세를 다양하게 움직여 20 점 이상의 위치를 측정하고 위치를 프로그램으로 기록하십시오. 

    * 이 때, 초기 3 점은 방향이 고정된 상태에서 하나의 직선 상에 존재하지 않도록 위치를 기록합니다. 
    
    * 일반적으로 측정 데이터의 개수가 많을수록 보다 정확한 캘리브레이션이 가능합니다.

{% hint style="warning" %}
* 프로그램 기록에 사용한 툴번호는 캘리브레이션 결과를 저장할 때, 툴길이가 변경될 수 있습니다. 툴길이가 변경되어도 무관한 툴번호를 선택하여 프로그램을 기록하십시오.
{% endhint %}

4. 로봇은 고정된 상태에서 베이스 축만 움직여 위치를 측정하고 위치를 프로그램으로 기록하십시오. 

    * 이 때, 동시에 두 축 이상의 베이스 축을 움직이지 마십시오. 
    
    * 기록 위치는 하나의 베이스 축 당 최소 5점 이상을 측정 및 기록하십시오.

    * 하나의 베이스 축에 대한 위치 측정 및 기록은 중간에 다른 베이스 축에 대한 위치가 포함되지 않도록 반드시 연속해서 기록하십시오. <br> 프로그램 상에 여러 베이스 축에 대한 기록 위치가 교대로 섞여 있으면 캘리브레이션이 정상적으로 수행될 수 없습니다.
    
    * 또한, 하나의 축 당 기록위치가 30점을 초과하거나 전체 기록 위치가 100점을 초과할 수 없습니다.

5. 측정한 위치 데이터를 X, Y, Z 형식으로 정리하여 파일(형식: ASCII, 확장자: MSR, 단위: mm)을 생성하십시오.
...
![지면에 놓인 로봇의 중력 방향\(좌\) / 경사면에 놓인 로봇의 중력 방향\(우\)](../../_assets/image_503.png)
...
![\[속성\] 버튼의 기능](../_assets/tp630/lbt-property-arc.png)
...
3. `[기록]` 키를 누르거나 `[ESC]` 키를 눌러 작업을 종료하십시오.
...
3. `[확인]` 버튼을 누르십시오. 작업 프로그램에 변경 내용이 저장되고 작업이 종료됩니다.
...
![스폿용접 펑션](../_assets/tp630/app-spot-menu.png)
