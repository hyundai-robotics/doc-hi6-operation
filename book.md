...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      의도하지 않은 화면 접촉에 의한 티치펜던트 오조작이 우려될 경우 이 옵션을 끄십시오.</td>
    </tr>
    <tr>
      <td style="text-align:left">
        조그키 사용 여부
      </td>
      <td style="text-align:left">조그키 `J7-`/`J7+`와 `J8-`/`J8+`를 사용할 지 각각 선택합니다. <br>조그키 오조작으로 인한 포지셔너 충돌 등이 우려될 경우 이 옵션을 끄십시오.<sup>1)</sup></td>
    </tr>
    <tr>
      <td style="text-align:left">
        언어
      ...
      (V70.00-00 버전부터 지원됩니다.<sup>2)</sup>) </td>
    </tr>
  </tbody>
</table>
...
1\) 조그키 사용 여부에 관한 자세한 내용은  "[7.6.6 메커니즘 설정](../7-system/6-initialization/6-mechannism-set.md)"의 메커니즘 조그 규칙을 참고하십시오.

2\) 그 이 전 버전은 `[F1: 서비스] - 9: TP 응용프로그램 종료` 를 수행한 상태에서만 표시 언어를 전환할 수 있습니다.
...
Default (청색 실선): 가속 시작과 종료 시점에 속도 변화가 급격하여 기계적 충격이 발생할 수 있습니다.
...
3.	체인지할 용접건 번호를 입력한 후 `[확인]` 버튼을 터치하거나 `[ENTER]` 키를 누르십시오. ${cont_model} 티치 펜던트 화면 좌측의 \[건\] 버튼에 선택한 용접건 번호가 표시됩니다.
