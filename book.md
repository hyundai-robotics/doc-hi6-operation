...
### 참고사항
소음은 데이터로 분석이 어려운 부분으로, 튜닝 담당자가 직접 소음을 들으며 튜닝하는 것 만큼 섬세하게 튜닝되기 어렵습니다. 튜닝된 게인이 소음이 날 경우, **시스템 > 3. 로봇 파라미터 > 33. 서보 파라미터 > 1. 서보 루프 게인** 메뉴로 진입하여 Kv 값을 낮춰가며(Kv 값이 바뀌면, 다른 게인 값들이 자동 계산되어 바뀜) 소음이 안나는 지점을 찾아야 합니다.  
