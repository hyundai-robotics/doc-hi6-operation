...
IO변수, 내장 PLC의 각 릴레이, 기타 일반 변수에 대해 주석을 등록할 수 있습니다. 등록된 주석은 모니터링창에 툴 팁으로 표시됩니다. (범용 입출력, 메모리, 전역변수, 각종 데이터 모니터링)
...
* [3.2.4.5 블록 편집 모드](../../3-programming/2-prog-edit/4-statement-edit/5-block-edit-mode.md) - `자동 주석` 기능.
...
### 데이터 주석 화면
   ...
   2. 상단 좌측의 필터 콤보박스들로 원하는 데이터 종류와 타입을 선택합니다.
...
### 이동
   1. 상하 화살표키를 눌러 항목을 이동할 수 있습니다. Ctrl키와 같이 누르면 더 빠르게 이동합니다.

   2. 혹은, `이름` 열에 숫자키로 인덱스를 입력하여 해당 인덱스로 바로 이동할 수도 있습니다. (`etc` 객체에서는 불가)

   3. 한번에 표시되는 항목 수는 최대 1000개입니다. 따라서, M릴레이 등 이보다 최대 인덱스가 큰 종류는 한번에 볼 수 없습니다. 위 2.의 방법으로 페이지를 바꿔가며 이동해야 합니다.
...
### `DataCmt.txt` 파일
   ...
   3. HRLadder의 `릴레이설명 가져오기`/`릴레이설명 내보내기` 기능의 파일형식과 호환됩니다. 따라서 작성된 파일은 Hi6/Hi7 제어기, HRLadder 공용으로 사용할 수 있습니다. (Hi5a 제어기와도 호환됩니다. 단, 릴레이명, 변수명의 차이는 조정이 필요합니다.)
